...
<html>
 ...
 $img_v = explode(',',$list['imagename_v']); array_pop($img_v); $title = explode(',',$list['types']); array_pop($title); $lianjie = explode(',',$list['video']); array_pop($lianjie); ?>
								...
								<li>
									...
									<p>标题：<?php echo $title[$i]?></p>
									</div>
									<div class="u-v-li-a-div1">
										<p>链接：<?php echo $lianjie[$i]?></p>
									</div>
								</li>
									<?php }?>
								<div class="clearfloat "></div>
					...
					<div class="s-c-4f-2">
						...
						<?php if(in_array($_SESSION['id'],$shou)){ ?>
							<div class="s-tf-btn"  id="bt-sc">
								<p style="left:0px;">已收藏
								</p>
							</div>
						<?php }else{ ?>
							<div class="s-tf-btn"  onclick="shou(<?php echo ($list["pid"]); ?>)" id="bt-sc">
								<p>收藏
								</p>
								<img src="/matouPCS/Public/Home/img/sc.png" style="top: 8px;">
							</div>
						<?php } ?>
						<div class="clearfloat "></div>
					</div>
