	...
	<script>
		...
		<script src="http://res.wx.qq.com/connect/zh_CN/htmledition/js/wxLogin.js"></script>
						...
						<div class="h-c-3f">
							...
							<div class="s-f1">
								<div class="yhtx">
											<?php if($bu[imagename]==''): ?><img src="/matouPCS/Public/Home/img/yhmc.png"/>
                                   <?php else: ?>
                                     <img style="width: 120px; height: 120px;border-radius:50%" src="/MatouPCS/Tu/upload/<?php echo ($bu["imagename"]); ?>"  /><?php endif; ?>
									<div class="vip">
							...
							<div class="s-f2">
								...
								<div>
									<p class="fsl">
										<span class="icon-fs"></span> <span id="gfen" style="font-size: 14px;"> <?php echo ($bu["fen"]); ?></span>
									</p>
									<p class="szd">
										<span class="szd-word"><?php echo ($bu["addre"]); ?></span>
							...
							<div class="s-f3">
								<?php if(is_array($uus)): foreach($uus as $key=>$uuss): if($uuss['uid'] == $bu['uid']){ ?>
									<?php  $uu = explode(',',$uuss['fid']); array_pop($uu); if(in_array($_SESSION['id'],$uu)){ ?>
										<button class="dy" id="gz-qx" onclick="guanzhu(<?php echo ($bu['uid']); ?>)">已关注</button>
									<?php }else{ ?>
										<?php if(empty($_SESSION['id'])){ ?>
											<button class="dy" id="gz-qx" onclick="alert('请先登录')">关注</button>
										<?php }else{ ?>
											<button class="dy" id="gz-qx" onclick="guanzhu(<?php echo ($bu['uid']); ?>)">关注</button>
										<?php } ?>
									<?php } ?>
							...
							<div class="s-f4">
								...
								<a id="zp" href="">作品</a>
								<a style="width: 105px;" class="red" href="rzbddyly-3r-xy.php">留言区</a>
								</div>
							</div>
						</div>
				</div>
							...
							<div class="s-c-4f-2f">
								<div class="jbxx">
									...
									<ul class="right">
										...
										<li><?php echo ($bu["type_bd"]); ?></li>
										<li><?php echo ($bu["address"]); ?></li>
										<li>
											<span class="tel"><?php echo (substr($bu["tels"],0,3)); ?>XXXX<?php echo (substr($bu["tels"],8,11)); ?></span>
											<button id="btnConfirm">查看</button>
											<input type="hidden" id="xp" value="<?php echo ($bu["tels"]); ?>">
										</li>
										<li><?php echo ($bu["qqs"]); ?></li>
										<li class="bdjj">
											<?php echo ($bu["content"]); ?>
										</li>
										<li><?php echo ($bu["lianjie"]); ?></li>
									</ul>
					...
					<?php if(is_array($list)): foreach($list as $key=>$v): ?><div class="s-c-3f-1f">
							...
							<div class="bottom">
								...
								<div class="right dz-qx">
										<p onclick="zan1(<?php echo ($v["lid"]); ?>,<?php echo ($v["zan"]); ?>)" class="dz dz-qx">
													<?php $zan = explode(',',$v['zid']); array_pop($zan); ?>
													<?php if(in_array($_SESSION['id'],$zan)){ ?>
													<span id="zz<?php echo ($v["lid"]); ?>" style="font-size: 15px;"class="icon-dz-kz"></span>
													<?php }else{ ?>
													<span id="zz<?php echo ($v["lid"]); ?>" style="font-size: 15px;" class="icon-dz"></span>
													<?php } ?>
													<span style="font-size: 15px;" id="ss<?php echo ($v["lid"]); ?>"><?php echo ($v["zan"]); ?></span></p> 
								</div>
								<p class="clearfloat"></p>
							</div>
							<div class="clearfloat"></div>
		...
		$.ajax({
			...
			success:function(data){
				...
				if(data!=null){				
						...
						$("#contentt").append('<div class="s-c-3f-1f"><div><div class="yhtx">'+url+img+'</a><div class="vip"><img src="/matouPCS/Public/Home/img/rzlogo.png" /></div><div class="clearfloat"></div></div><p class="yhmc"><a href="">'+use+'</a></p><p class="zwmc"> 职位名称： '+data[i].type_u+'</p><p class="szd">所在地：'+data[i].addre+'</p><div class="clearfloat"></div></div><p class="dtnr"> '+data[i].content+'</p><div class="bottom"><p class="left">'+data[i].time+'</p><!--<span class="delete"><img src="img/delete-2.png" /></span>--><div class="right dz-qx"><p onclick="zan1('+data[i].id+','+data[i].zan+')" class="dz dz-qx">'+dianzan+'</span><span style="font-size: 15px;" id="ss'+data[i].id+'">'+data[i].zan+'</span></p></div><p class="clearfloat"></p></div><div class="clearfloat"></div></div>');
					...
					if(aa==1){
					 ...
					 if(data[i].username==data[i].tel){
					   ...
					   if(t==data[i].uid){
						...
						li+='<div class="s-c-3f-1f"><div><div class="yhtx">'+url+img+'</a><div class="vip"><img src="/matouPCS/Public/Home/img/rzlogo.png" /></div><div class="clearfloat"></div></div><p class="yhmc"><a href="">'+use+'</a></p><p class="zwmc"> 职位名称： '+data[i].type_u+'</p><p class="szd">所在地：'+data[i].addre+'</p><div class="clearfloat"></div></div><p class="dtnr"> '+data[i].content+'</p><div class="bottom"><p class="left">'+data[i].time+'</p><!--<span class="delete"><img src="img/delete-2.png" /></span>--><div class="right dz-qx"><p onclick="zan1('+data[i].id+','+data[i].zan+')" class="dz dz-qx">'+dianzan+'</span><span style="font-size: 15px;" id="ss'+data[i].id+'">'+data[i].zan+'</span></p></div><p class="clearfloat"></p></div><div class="clearfloat"></div></div>';
				...
				$.ajax({
					...
					success:function(data){
						...
						if(data!=null){				
							...
							$("#ha").append('<div class="s-c-3f-1f"><div><div class="yhtx">'+url+img+'</a><div class="vip"><img src="/matouPCS/Public/Home/img/rzlogo.png" /></div><div class="clearfloat"></div></div><p class="yhmc"><a href="">'+use+'</a></p><p class="zwmc"> 职位名称： '+data[i].type_u+'</p><p class="szd">所在地：'+data[i].addre+'</p><div class="clearfloat"></div></div><p class="dtnr"> '+data[i].content+'</p><div class="bottom"><p class="left">'+data[i].time+'</p><!--<span class="delete"><img src="img/delete-2.png" /></span>--><div class="right dz-qx"><p onclick="zan1('+data[i].id+','+data[i].zan+')" class="dz dz-qx">'+dianzan+'</span><span style="font-size: 15px;" id="ss'+data[i].id+'">'+data[i].zan+'</span></p></div><p class="clearfloat"></p></div><div class="clearfloat"></div></div>');
